...
char *SSQ::jieya(int type)
{
	 ...
	 mystl::my_string suoS = //  619-01-21开始16598个朔日修正表 d0=1947168
...
int SSQ::leap;  //闰月位置
mystl::static_array<mystl::my_string,14> SSQ::ym;//各月名称索引
int SSQ::ZQ[25];//中气表
int SSQ::HS[15];//合朔表
int SSQ::dx[14];//各月大小
int SSQ::pe[2]; //补算二气
...
{//农历排月序计算,可定出农历,有效范围：两个冬至之间(冬至一 <= d < 冬至二)
    ...
    for (i=0;i<14;i++)
    {
        ...
        mystl::my_string mc = str_ymc[v2%12]; //月建对应的默认月名称：建子十一,建丑十二,建寅为正……
