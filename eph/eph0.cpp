...
{//计算世界时与原子时之差,传入年
	double dt_at[] = { -4000, 108371.7, -13036.80, 392, 0, -500, 17201.0, -627.82, 16.170, -0.3413, -150, 12200.6, -346.41, 5.403, -0.1593, 150, 9113.8, -328.13, -1.647, 0.0377, 500, 5707.5, -391.41, 0.915,
			0.3145, 900, 2203.4, -283.45, 13.034, -0.1778, 1300, 490.1, -57.35, 2.085, -0.0072, 1600, 120.0, -9.81, -1.532, 0.1403, 1700, 10.2, -0.91, 0.510, -0.0370, 1800, 13.4, -0.72, 0.202,
			-0.0193, 1830, 7.8, -1.81, 0.416, -0.0247, 1860, 8.3, -0.13, -0.406, 0.0292, 1880, -5.4, 0.32, -0.183, 0.0173, 1900, -2.3, 2.06, 0.169, -0.0135, 1920, 21.2, 1.69, -0.304, 0.0167, 1940,
			24.2, 1.22, -0.064, 0.0031, 1960, 33.2, 0.51, 0.231, -0.0109, 1980, 51.0, 1.29, -0.026, 0.0032, 2000, 63.87, 0.1, 0, 0, 2005, 64.7, 0.4, 0, 0, 2015, 69 };
...
{//计算月亮
	...
	int len0[] = { 4, 3, 3 };
	int len_ML[] = {
		sizeof(ML0) / sizeof(double),
		sizeof(ML1) / sizeof(double),
		sizeof(ML2) / sizeof(double),
		sizeof(ML3) / sizeof(double)
	};
	int len_MB[] = {
		sizeof(MB0) / sizeof(double),
		sizeof(MB1) / sizeof(double),
		sizeof(MB2) / sizeof(double)
	};
	int len_MR[] = {
		sizeof(MR0) / sizeof(double),
		sizeof(MR1) / sizeof(double),
		sizeof(MR2) / sizeof(double)
	};
	int *len_M[] = { len_ML, len_MB, len_MR };
...
{//月球速度计算,传入世经数
	...
	v -= 179 * sin(2.543 + 15542.7543 * t)	//误差小于0.3%
		...
		+ 7 * sin(0.23 + 6585.7609 * t) + 5 * sin(0.9 + 25195.624 * t) + 5 * sin(2.32 - 7700.3895 * t) + 5 * sin(3.88 + 8956.9934 * t) + 5 * sin(0.49 + 7771.3771 * t);
	return v;
}
